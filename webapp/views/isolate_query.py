...
def isolate_query_api_view(request):
   ...
   # isolatesRef = Base.classes.isolate_pub_ref
    ...
    for items in query_container:
        ...
        if operatr == "=":
            query = (
                request.db2_session.query(isolates_table)
                .filter(models == value)
                .first()
            )
            if query:
            ...
            query = (
                request.db2_session.query(isolates_table)
                .filter(models.like("%{}%".format(value)))
                .first()
            )
            if query:
                if combo == "AND":
...
@view_config(
    route_name="isolate_query_fc", renderer="../templates/isolate_query_faceted.jinja2"
)
