...
class NewickProcessor:
    ...
    def distance_matric(self, df, dc=0):
        ...
        for index_i in range(nr):
            ...
            for index_j in range(start, nr):
                result[pos] = self.shriver_distance(
                    df_array[j_counter], df_array[start]
                )
                pos += 1
                start += 1
        return result
    ...
    def linkage_to_newick(self, Z, labels):
        ...
        def buildNewick(node, newick, parentdist, leaf_names):
            if node.is_leaf():
                ...
                newick = buildNewick(
                    node.get_right(), ",%s" % (newick), node.dist, leaf_names
                )
                newick = "(%s" % (newick)
                return newick
    ...
    def write_newick(process_ID, newick):
        with open(
            "/home/ubuntu/coxbase/tools/grapeTree/tre/{}.nwk".format(process_ID), "wb"
        ) as open_file:
    ...
    def write_metadata(process_ID, metadata_list):
        with open(
            "/home/ubuntu/coxbase/tools/grapeTree/tre/{}.txt".format(process_ID),
            "w",
            newline="",
            encoding="utf-8",
        ) as open_file:
    ...
    def create_itol_link(process_ID, newick):
        with open(
            "/home/ubuntu/coxbase/tools/grapeTree/tre/{}.tree".format(process_ID),
            "w",
            encoding="utf-8",
        ) as open_file:
