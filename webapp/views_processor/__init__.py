...
class ViewProcessor():
    ...
    def create_file_from_fastafile_combined(inputfile, process_ID):
        ...
        file_path = os.path.join('/home/ubuntu/temp/combined/{}'.format(process_ID),'{}.fasta'.format(process_ID))
        temp_file_path = file_path
        inputfile.seek(0)
        with open(temp_file_path, 'wb') as output_file:
            shutil.copyfileobj(inputfile, output_file)
    ...
    def create_epcr_command_combined(file_path, process_ID):
        out_file = "/home/ubuntu/temp/combined/{}/epcr_mlva_{}_output.tab".format(process_ID,process_ID)
        Path(out_file).touch()
        sts_file = "/home/ubuntu/coxbase/coxbase/scripts/input/coxiella_ms.sts"
        command = ["/home/ubuntu/tools/Linux-x86_64/e-PCR", "-w7", "-f","1","-m100","-o", out_file,"-t3", sts_file, file_path]
        return command
    ...
    def create_epcr_command_is1111_combined(file_path, process_ID):
        out_file = "/home/ubuntu/temp/combined/{}/epcr_is1111_{}_output.tab".format(process_ID,process_ID)
        Path(out_file).touch()
        sts_file = "/home/ubuntu/coxbase/coxbase/scripts/input/coxiella_IS.sts"
        command = ["/home/ubuntu/tools/Linux-x86_64/e-PCR", "-w7", "-f","1","-m100","-o", out_file,"-t3", sts_file, file_path]
        return command
    ...
    def mstprocessor_combined(file_path, process_ID):
        ...
        for key, value in primer_dict.items():
            ...
            fwd_prim = value[0]
            rev_prim = value[1]
            out_file = os.path.join("/home/ubuntu/temp/combined/{}/mlva".format(process_ID),
                                    "{}.fasta".format(spacer))
            command = ["/home/ubuntu/tools/usearch",
                   "-search_pcr2",file_path,"-fwdprimer", fwd_prim, "-revprimer",
                   rev_prim,"-strand", "both", "-fastaout", out_file]
    ...
    def adaprocessor_combined(file_path, process_ID):
        ...
        for key, value in primer_dict.items():
            fwd_prim = value[0] # forward primer
            rev_prim = value[1] # reverse primer
            out_file = os.path.join("/home/ubuntu/temp/combined/{}/ada".format(process_ID),
                                    "{}.fasta".format(key))
            command = ["/home/ubuntu/tools/usearch",
                   "-search_pcr2",file_path,"-fwdprimer", fwd_prim, "-revprimer",
                   rev_prim,"-strand", "both", "-fastaout", out_file]
